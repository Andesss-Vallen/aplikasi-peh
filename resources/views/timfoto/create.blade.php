...
@section('content')
    <div class="container">
        <h1>Tambah Tim Foto</h1>
        <h4>Form untuk menambahkan Tim Foto baru</h4>
        ...
        <form action="{{ route('timfoto.store') }}" method="POST">
            @csrf
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <select name="status_tfoto" class="form-select" id="status_tfoto" required>
                        <option value="1" {{ old('status_tfoto') == '1' ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ old('status_tfoto') == '0' ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>
            </div>
